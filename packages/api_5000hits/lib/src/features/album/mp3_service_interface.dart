...
abstract class Mp3AlbumServiceInterface {
  ...
  Future<PaginationResult<Mp3Album>> searchAlbums(String searchTerm, {int page = 0, int pageSize = 20});
  Future<Mp3Album> getAlbumDetails(String slug);

  /// Récupère un album spécifique par son slug.
  /// Vérifie d'abord le cache local, puis fait une requête distante si nécessaire.
  ...
  Future<PaginationResult<Mp3Album>> getPopularAlbums({int page = 0, int pageSize = 20});

  /// Récupère les albums récents.
  /// Fait une requête distante et met à jour le cache local.
  Future<PaginationResult<Mp3Album>> getRecentAlbums({int page = 0, int pageSize = 20});
  ...
  Future<PaginationResult<Mp3Album>> getAlbumsByGenre(String genre, {int page = 0, int pageSize = 20});

  /// Récupère les albums par artiste.
  /// Utilise d'abord les données locales, puis met à jour avec les données distantes si nécessaire.
  Future<PaginationResult<Mp3Album>> getAlbumsByArtist(String artist, {int page = 0, int pageSize = 20});

  /// Récupère les albums similaires à un album donné.
  /// Combine les résultats locaux et distants pour une meilleure pertinence.
  Future<PaginationResult<Mp3Album>> getSimilarAlbums(Mp3Album album, {int page = 0, int pageSize = 20});
