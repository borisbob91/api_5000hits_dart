...
class PopulareAlbums extends StatefulWidget {
  @override
  _AlbumListPageState createState() => _AlbumListPageState();
}

class _AlbumListPageState extends State<PopulareAlbums> {
  ...
  Future<List<Mp3Album>> _fetchAlbums() async {
    ...
    try {
      List<Mp3Album> result = await _musicSdk.albumContrat?.getPopularAlbums(page: _currentPage) ?? [];
      if (result.isEmpty) {
        _currentPage = -1;
        return [];
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Albums Populaires'),
        actions: [
          IconButton(
